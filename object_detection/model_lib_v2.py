      ...
      with tf.compat.v2.summary.record_if(
        ...
        for _ in range(global_step.value(), train_steps,
          ...
          if global_step.value() - logged_step >= NUM_STEPS_PER_ITERATION:
            ...
            print(f"STEP :: {global_step.value():06} || " +                              \
                  f"TIME :: {time_taken / num_steps_per_iteration:.4f} || " +            \
                  f"CLA. L :: {logged_dict_np['Loss/classification_loss']:.3f} || "  +   \
                  f"LOC. L :: {logged_dict_np['Loss/localization_loss']:.3f} || "  +     \
                  f"REG. L :: {logged_dict_np['Loss/regularization_loss']:.3f} || " +    \
                  f"TOT. L :: {logged_dict_np['Loss/total_loss']:.3f} || " +             \
                  f"LR :: {logged_dict_np['learning_rate']:.4f}")
            logged_step = global_step.value()
